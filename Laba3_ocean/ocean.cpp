...
void Ocean_master::Tick()
{
	...
	for (int i = 0; i < killerwhale_count; i++)
		for (int j = 0; j < fish_killerwhale[i].moves; j++)
	...
	for (int i = 0; i < shark_count; i++)
		for (int j = 0; j < fish_shark[i].moves; j++)
...
void Ocean_master::Neutral_eat(Neutral* c_obj)
{
	...
	if (c_obj->food <= 0.2 * c_obj->food_max)
	{
		...
		if (x_fugu == x && y_fugu == y)
		{
			...
			if (c_obj->food > c_obj->food_max)
				c_obj->food = c_obj->food_max;
			c_obj->hp -= 20; //ÿä ðûáû ôóãó îòíèìàåò ìíîãî õï
...
void Ocean_master::Neutral_replicate(Neutral* c_obj)
{
	...
	if (c_obj->type == 'W')
	{
		if (c_obj->food >= (c_obj->food_max) * 0.9)
		{
			for (int i = 0; i < fish_whale.size(); i++)
			{
...
void Ocean_master::Aggressive_move(Aggressive* c_obj)
{
	...
	if (c_obj->food <= 0.4 * c_obj->food_max)
	{
		Dolphin dolphin_obj = return_dolphin(x, y);
		Whale whale_obj = return_whale(x, y);
		if (dolphin_obj.location[0] == x && dolphin_obj.location[1] == y)
			return;
		if (whale_obj.location[0] == x && whale_obj.location[1] == y)
			return;
	}
	...
	if (c_obj->food <= 0.8 * c_obj->food_max)
	{
		Clownfish clownfish_obj = return_clownfish(x, y);
		Fugu fugu_obj = return_fugu(x, y);
		if (fugu_obj.location[0] == x && fugu_obj.location[1] == y)
			return;
		if (clownfish_obj.location[0] == x && clownfish_obj.location[1] == y)
			return;
	}
...
void Ocean_master::Aggressive_eat(Aggressive* c_obj)
{
	...
	if (c_obj->food <= 0.4 * c_obj->food_max)
	{
		Dolphin dolphin_obj = return_dolphin(x, y);
		Whale whale_obj = return_whale(x, y);
		int x_dolphin = dolphin_obj.location[0], y_dolphin = dolphin_obj.location[1];
		int x_whale = whale_obj.location[0], y_whale = whale_obj.location[1];

		if (x_dolphin == x && y_dolphin == y)
		{
			if (c_obj->damage >= dolphin_obj.hp)	// KILLING DOLPHIN
			{
				if (c_obj->food + c_obj->damage > c_obj->food_max)
					c_obj->food += c_obj->food_max - c_obj->food;
				else
					c_obj->food += c_obj->damage;
				ocean_table->scene[x_dolphin][y_dolphin][dolphin_obj.location[2]] = ' ';
				for (int i = 0; i < fish_dolphin.size(); i++)
				{
		...
		{
			if (c_obj->damage >= whale_obj.hp)	// KILLING WHALE
			{
				if (c_obj->food + c_obj->damage > c_obj->food_max)
					c_obj->food += c_obj->food_max - c_obj->food;
				else
...
void Ocean_master::Aggressive_replicate(Aggressive* c_obj)
{
	...
	if (c_obj->type == 'S')
	{
		if (c_obj->food >= (c_obj->food_max) * 0.9)
		{
			for (int i = 0; i < fish_shark.size(); i++)
			{
				if (fish_shark[i].location[0] == x1 && fish_shark[i].location[1] == y1)
				{
					if (fish_shark[i].location[2] != c_obj->location[2]) // if they are not in the same sub-cell
					{
						if (c_obj->isMale == fish_shark[i].isMale)
							return;
						replica_result = empty_place(x1, y1, 0);
						if (replica_result == -1)
							return;
	...
	if (c_obj->type == 'K')
	{
		if (c_obj->food >= (c_obj->food_max) * 0.9)
		{
			for (int i = 0; i < fish_killerwhale.size(); i++)
			{
				if (fish_killerwhale[i].location[0] == x1 && fish_killerwhale[i].location[1] == y1)
				{
					if (fish_killerwhale[i].location[2] != c_obj->location[2]) // if they are not in the same sub-cell
					{
						if (c_obj->isMale == fish_killerwhale[i].isMale)
							return;
						replica_result = empty_place(x1, y1, 0);
						if (replica_result == -1)
							return;
